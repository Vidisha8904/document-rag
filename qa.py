...
def get_pdf_text(pdf_docs):
    """Extracts text from PDFs and stores metadata."""
    documents = []
    for pdf in pdf_docs:
        pdf_reader = PdfReader(pdf)
        text = ""
...
def get_text_chunks(documents):
    """Splits text into chunks with overlap and saves to a text file."""
    text_splitter = RecursiveCharacterTextSplitter(
        chunk_size=500,
        chunk_overlap=150,
        separators=["\n", ".", "?", "!", ":"],  # Preserve Q&A format
    ...
    all_chunks = []
    with open("chunks.txt", "w", encoding="utf-8") as f:  # Open file to save chunks
        for doc in documents:
            chunks = text_splitter.create_documents(
                texts=[doc.page_content],
                metadatas=[doc.metadata]
    ...
    print("[INFO] Chunks saved to chunks.txt")  # Confirmation log
    return all_chunks   
...
def get_vector_store(chunks):
    ...
    # Debug logs
    print(f"[INFO] Created vector store with {len(chunks)} chunks")
    st.sidebar.write(f"Created vector store with {len(chunks)} chunks")
...
def user_input(user_question):
    """Handles user queries and retrieves answers."""
    embeddings = OpenAIEmbeddings(model="text-embedding-ada-002")
    new_db = FAISS.load_local("faiss_index", embeddings, allow_dangerous_deserialization=True)
    ...
    # Debug logs: Print retrieved docs
    ...
    for i, doc in enumerate(docs):
        ...
        print(doc.page_content[:300])  # Print first 300 characters of each retrieved chunk
        st.sidebar.write(f"- From: {doc.metadata.get('source', 'Unknown')}")

    sources = list(set(doc.metadata.get('source', 'Unknown') for doc in docs))
    sources_str = ', '.join(sources)

    # Create system message with debugging
    messages = [
        {
            "role": "system",
            "content": f"""You are an AI assistant that helps users understand PDF documents. 
    ...
    # Debug logs
    print("\n[INFO] Sending messages to GPT-4o:")
    for msg in messages:
...
def display_chat_message(role, content):
    """Displays chat messages in a styled format."""
    with st.container():
        if role == "user":
            st.markdown(f"""
            <div style="display: flex; justify-content: flex-end;">
