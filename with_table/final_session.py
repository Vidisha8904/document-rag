# working fine no errors, creates new session and do not take past sessions data.
...
if 'session_start_time' not in st.session_state:
    # Check if old data exists and remove it (assuming it's from a previous browser session)
    ...
    # Set the start time for this new session
...
def user_input(user_question):
    if not os.path.exists("faiss_index") or not os.path.exists("faiss_index/index.faiss"):
        return "Please upload and process PDF files first before asking questions."
    ...
    try:
        ...
        return "An error occurred while processing your question. Please make sure you've uploaded and processed PDF files first."
...
def main():
    ...
    # In your main() function, near the input box
    if os.path.exists("faiss_index") and os.path.exists("faiss_index/index.faiss"):
        ...
        st.text_input("Ask any Question from the PDF Files", 
                    key="user_input", 
                    on_change=handle_submit,
                    disabled=False)  # You can set this to True to disable input until PDFs are processed
